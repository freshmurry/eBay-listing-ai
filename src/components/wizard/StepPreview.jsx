...
export default function StepPreview({ project, updateProject, onPrev, isLastStep }) {
    ...
    const generateHtml = async () => {
      ...
      try {
        ...
        if (project.status !== "COMPLETED" || project.htmlPreview !== html) {
          ...
          // Track usage for completed listing generation (only on first completion)
          if (project.status !== "COMPLETED") {
            await incrementUsage('listings');
          }
        }
      } catch (error) {
  ...
  const generateMasterTemplate = (projectData) => {
    console.log('Generating template with project data:', projectData);
...
<head>
    ...
    <script type="application/ld+json">
    {
      ...
      "image": ${JSON.stringify((projectData.images || []).map(img => typeof img === 'string' ? img : img.url))},
    ...
    <section class="mb-8">
        <div class="image-grid-container">
            ...
            <div class="image-grid-item ${index === 0 ? 'main-image' : ''}">
                <img src="${imageUrl}" alt="${projectData.title} - Image ${index + 1}" class="w-full h-full object-cover rounded-md" onerror="this.style.display='none'">
            </div>
            `;
            }).filter(Boolean).join('')}
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Alert className="mb-6 border-emerald-200 bg-emerald-50">
          ...
          <AlertDescription className="text-emerald-800">
            <strong>Description Completed!</strong> Your professional eBay description is ready to use.
          </AlertDescription>
        </Alert>
